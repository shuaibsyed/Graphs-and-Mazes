...
public class WeightedGraph<V> {
	...
	public void DoBFS(V start, V end) {
		...
		Set<V> visitedSet = new HashSet<>(); //Set for visited vertices
		Queue<V> queue = new LinkedList<>(); // Queue for vertices
		queue.add(start); // begins the search

		while(!(queue.isEmpty())) {
			V current = queue.element(); // visit vertex
			queue.remove(current); //remove from queue
			if(!(visitedSet.contains(current))) {
				for(GraphAlgorithmObserver<V> observer : observerList) {
					observer.notifyVisit(current);
				}
				visitedSet.add(current); // adds vertex to visitedSet
				Map<V, Integer> adjacencyList = weightedGraph.get(current);

				for(V currentVertNeighbor: adjacencyList.keySet()) { //repeat process for all adjacencies
					if(!(visitedSet.contains(currentVertNeighbor))) {
						queue.add(currentVertNeighbor);
					}
				}
			}
			if(current.equals(end)) {
				...
				break; //once its at the end break out of loop to finish search
			}
		}
	...
	public void DoDFS(V start, V end) {
		...
		Set<V> visitedSet = new HashSet<>(); //Set for visited vertices
		Stack<V> stack = new Stack<>(); //stack for vertices
		stack.add(start); //starts the search
		...
		while(!(stack.isEmpty())) {
			...
			if(!(visitedSet.contains(current))) {
				...
				visitedSet.add(current); //add to visitedSet
				Map<V, Integer> adjacencyList = weightedGraph.get(current);

				for(V currentVertNeighbor: adjacencyList.keySet()) { //repeat process for all adjacencies
					if(!(visitedSet.contains(currentVertNeighbor))) {
						stack.push(currentVertNeighbor);
					}
				}
			}
			if(current.equals(end)) {
	...
	public void DoDijsktra(V start, V end) {
		...
		Set<V> visitedSet = new HashSet<>(); //Set for visited vertices
		HashMap<V, Integer> costMap = new HashMap<>(); // Cost list
		HashMap<V, V> predMap = new HashMap<>(); //Predecessor list
		costMap.put(start, 0); //starts Dijsktra

		for(V currentVert : weightedGraph.keySet()) {
			if(!(currentVert.equals(start))) {
				costMap.put(currentVert, 10000000); // sets all other vertices to massive numbers
			}
		}

		for(V currentVert : weightedGraph.keySet()) {
			predMap.put(currentVert, null); //sets the predecessors to null
		}
		...
		while(visitedSet.size() != weightedGraph.size()) {
			...
			for(V currentVertex : costMap.keySet()) {
				if(visitedSet.contains(currentVertex) == false) {
					if(costMap.get(currentVertex) < smallest) {  //checks current cost if it has a smaller value
						smallest = costMap.get(currentVertex); //if so gets replaced 
						smallVertexUp = currentVertex;          
					}
				}
			}
			/* That block checks if value of currentVertex is smaller than stored value. If so it gets replaced
			 * 
			 */
			...
			for(GraphAlgorithmObserver<V> element: observerList) {
				element.notifyDijkstraVertexFinished(smallVertexUp, costMap.get(smallVertexUp)); 
			}
			//Smallest vertex now visited


			for(V currentVertex: weightedGraph.get(smallVertexUp).keySet()) { 
				//Now doing the neighbors of that smallest vertex
				if(visitedSet.contains(currentVertex) == false) {
					if(costMap.get(smallVertexUp) + this.getWeight(smallVertexUp, currentVertex) < 
							costMap.get(currentVertex)) {
						//Checking if the weight of the neighbor is smaller than the ones saved
						costMap.put(currentVertex, costMap.get(smallVertexUp) + this.getWeight(smallVertexUp, 
								currentVertex) );
						//If so now replacing that value with a new value 
		...
		V makeList = end;
		ArrayList<V> fastPath = new ArrayList<>(); //Creating the shortestPath at the end
		while(makeList != null) {
